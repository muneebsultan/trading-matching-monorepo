...
from Core.handle_sl_and_tp_with_trades import SlAndTpWithTrades 
...
class DatabaseManager:
    def __init__(self, use_nats=True):
        ...
        """
        self.alert = AlertForRecalculatePortfolio()
    ...
    def select(self, collection_name, filter_criteria, sort_criteria=None, mongodb_client="Traderverse-Authentication"):
        """
    ...
    def update_mongo_record(self, trade_id, remaining_quantity, executed_quantity, price, total_executed_qty, total_executed_value,event, collection_name=trade_table, mongodb_client="Traderverse-Authentication", user_id = None, user_requested_quantity = None, portfolio_id = None, dict_for_sltp = None):
        try:
            """
            ...
            if remaining_quantity == 0:
                self.insert_all_transaction_records(trade_id, event, weighted_avg_price, status="Filled")
                # alert_response = self.alert.alert_response(portfolioId=event.get("portfolio_id"))

                try:
                    loop = asyncio.get_event_loop()
                    if loop.is_running():
                        # Already in an event loop; schedule as a task
                        asyncio.create_task(self.alert.alert_response(portfolioId=event.get("portfolio_id")))
                    else:
                        # No running loop; run it directly
                        loop.run_until_complete(self.alert.alert_response(portfolioId=event.get("portfolio_id")))
                except RuntimeError as e:
                    # No event loop available at all
                    asyncio.run(self.alert.alert_response(portfolioId=event.get("portfolio_id")))
                except Exception as e:
                    logger_object['error'].log(f"❌ Error running Recalculating general Filled: {e}")

                try:
                    self.update(
                        collection=transaction_table,
                        filer={"trade_id": str(event.get("trade_id"))},
                        operation={
                    ...
                    logger_object['error'].log(f"❌ Error in SlAndTpWithTrades {trade_id}: {str(e)}")
                
                try:
                    stoploss = dict_for_sltp.get("stoploss", None)
                    takeprofit = dict_for_sltp.get("takeprofit", None)
                    user_id = dict_for_sltp.get("user_id", None)
                    portfolio_id = dict_for_sltp.get("portfolio_id", None)
                    stockId = dict_for_sltp.get("stockId", None)
                    symbol = dict_for_sltp.get("symbol", None)
                    asset_type = dict_for_sltp.get("asset_type", None)
                    createdBy = dict_for_sltp.get("createdBy", None)
                    slandtptrade = SlAndTpWithTrades(stoploss=stoploss, takeprofit=takeprofit, user_id=user_id, portfolio_id=portfolio_id, stockId=stockId, symbol=symbol, createdBy=createdBy, asset_type=asset_type)
                    slandtptrade.process_sl_and_tp(insert=self.insert)
                except Exception as e:
                    logger_object['error'].log(f"❌ Error in SlAndTpWithTrades {trade_id}: {str(e)}")
    ...
    def update_mongo_all_records(self, trade_id, event,price=None,status="Pending", collection_name=transaction_table, mongodb_client="Traderverse-Authentication"):
        ...
        order_details = {
            ...
            "createdBy": event.get("createdBy").capitalize() if event.get("createdBy") else None,
            "asset_type": event.get("asset_type").lower() if event.get("asset_type") else None,
            "stockId": event.get("stockId") if event.get("stockId") else None,
            "stop_loss": event.get("stop_loss") if event.get("stop_loss") else None, 
            "take_profit": event.get("take_profit") if event.get("take_profit") else None,
            "created_at": datetime.utcnow(),
        }
    ...
    def insert_all_transaction_records(self, trade_id, event, price=None, status="Pending", collection_name=transaction_table, mongodb_client="Traderverse-Authentication"):
        """
        Insert or update a trade record in MongoDB for historical tracking.
        """
        # Check if the record already exists in the collection
