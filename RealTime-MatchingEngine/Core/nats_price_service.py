    ...
    def fetch_quote(self, symbol):
        ...
        try:
            ...
            logger_object['success'].log(
                f"NATS data for {symbol}: OHLC={ohlcv['price']}/{ohlcv['volume']}/{ohlcv['ts']}/{ohlcv['symbol']}"
            )
            return quote
            # return {'symbol': 'MSFT', 'price': 464.34, 'volume': 10, 'source': 'NASDAQ', 'timestamp_UTC': datetime.now(timezone.utc)}
    ...
    def get_latest_stock_price(self, symbol, user_trade_time=None):
        ...
        try:
            ...
            result = [
                latest_time,
                symbol,
                float(latest_quote.get('price', 0)),
